...
void  CupDetconfig(void)  //自检初始化
   {
			...
			Cup1Waiting=1;Cup1Runing=0;Cup1Ending=0;Cup1EndingDraw=0;//状态判断/运行/结束/结束标志|赋值
...
if(Cup1Eff==1)
{
     if(Cup1Waiting&&(Cup1Eff==1))                                                       //由于系统自检将cup1waiting 置1了
	   { 
       ...
       if(SensorCupOut(1)==NoExist) //检测到杯子已经拿开|所有传感器和led灯全部回到原始状态|结束标志为高
        { 
           SurplusCount+=1;//杯子拿走剩余杯子计数器加1
					 ...
					 RX1_Buffer[26]=0x00;//杯子已使用数组值清零，此杯子只能使用一次
					 Cup1Waiting=0;
					 Cup1Runing=0;
					 Cup1Ending=0;
					 Cup1Eff=0;
       ...
       if(SensorCupOut(3)==NoExist) //检测到杯子已经拿开
        { 
           SurplusCount+=1;//杯子拿走剩余杯子计数器加1
					...
					RX1_Buffer[27]=0x00;//杯子已使用数组值清零，此杯子只能使用一次
					 Cup3Eff=0;
					 Cup3Waiting=0;
					 Cup3Runing=0;
					 Cup3Ending=0;
       ...
       if(SensorCupOut(5)==NoExist) //检测到杯子已经拿开
        { 
           SurplusCount+=1;//杯子拿走剩余杯子计数器加1
					 ...
					 RX1_Buffer[28]=0x00;//杯子已使用数组值清零，此杯子只能使用一次
					 Cup5Eff=0; 
					 Cup5Waiting=0;
					 Cup5Runing=0;
					 Cup5Ending=0;
