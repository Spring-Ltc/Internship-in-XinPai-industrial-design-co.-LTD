...
u8 EEPROM_read_n(u16 DataAddress)//读函数
{
	u8 dat;
	EA = 1;		//禁止中断
...
//	{
	//  IAP_CMD = CMD_READ;             //设置IAP命令
		ISP_ADDRH = DataAddress / 256;		//送地址高字节（地址需要改变时才需重新送地址）
		ISP_ADDRL = DataAddress % 256;		//送地址低字节
	
	  ISP_CMD=1;//写字节模式
		ISP_TRIG = 0x5a;                //写触发命令(0x5a)
    ISP_TRIG = 0xa5;                //写触发命令(0xa5)
	//	ISP_TRIG();							//先送5AH，再送A5H到ISP/IAP触发寄存器，每次都需要如此
											//送完A5H后，ISP/IAP命令立即被触发启动
											//CPU等待IAP完成后，才会继续执行程序。
		_nop_();
		dat  = ISP_DATA;			//读出的数据送往等待ISP/IAP/EEPROM操作完成
//		EE_address++;
//	DataAddress++;
//	}while(--number);
...
// 版本: V1.0, 2013-5-10
//========================================================================
void EEPROM_SectorErase(u16 EE_address)//擦除一个扇区
{
	EA = 0;		//禁止中断
...
// 版本: V1.0, 2012-10-22
//========================================================================
void  EEPROM_write_n(u16 addr,u8 dat)//写函数
{
	 // EA = 1;		//禁止中断

	  ISP_CONTR = (ISP_EN + ISP_WAIT_FREQUENCY);	//设置等待时间，允许ISP/IAP操作，送一次就够
	  ISP_WRITE();							//送字节写命令，命令不需改变时，不需重新送命令
		ISP_ADDRH = addr / 256;		//送地址高字节（地址需要改变时才需重新送地址）
		ISP_ADDRL = addr % 256;		//送地址低字节
		ISP_DATA  = dat;			//送数据到ISP_DATA，只有数据改变时才需重新送
		//ISP_TRIG();
	  ISP_CMD=2;//写字节模式
		ISP_TRIG = 0x5a;                //写触发命令(0x5a)
    ISP_TRIG = 0xa5;                //写触发命令(0xa5)
