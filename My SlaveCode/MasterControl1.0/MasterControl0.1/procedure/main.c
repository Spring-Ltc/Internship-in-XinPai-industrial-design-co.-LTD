...
//			for(i=0;i<=30;i++)
//						{
//							UART_Send_Byte(MasterDataBuff[i]);//调试串口buff数据
//						}	
    ...
    if(TaskUploadFlag!=0)//上传状态任务允许工作
		{
//			if(TaskUploadFlag==1)
//			{				//解析完成后，直接发送状态数据区的所有内容，或读取状态数据区只发送次数内容
			FramePacking(0x71,SlaveDataBuff);
			TaskUploadFlag = 0;//清除标志位
//			}
//			else if(TaskUploadFlag==2)
//			{
//			 FramePacking(0x71,SlaveDataBuff);
//			TaskUploadFlag = 0;//清除标志位
//			}
//			else if((TaskUploadFlag==3)&&(MasterDataBuff[20]!=0))
//			{
//				
//				if(vt1>=((u8)(MasterDataBuff[20])))
//				{
//				vt1=0;
//				FramePacking(0x71,SlaveDataBuff);
//				}
