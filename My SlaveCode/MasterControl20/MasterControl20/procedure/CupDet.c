...
void PumpControl()
{
		if((Cup1Status==1) && ((MasterDataBuff[12]==0x01) || (MasterDataBuff[12]==0x02)))//有杯子状态确定是否开启泵 
				Pump1=1;
			 else 
			 {		
					Pump1=0;//关泵
					if(MasterDataBuff[12] == 0)			
					{
							EXValve1=VALVEON;               //排空阀动作
							Delay1000ms();Delay1000ms();Delay1000ms();    //延时1s
							EXValve1=VALVEOFF;            //关阀门
							MasterDataBuff[12] = 0x03;
					} 
			 }
		 
		if((Cup3Status==1) && ((MasterDataBuff[13]==0x01) || (MasterDataBuff[13]==0x02)))//有杯子状态确定是否开启泵 
				Pump3=1;
			 else 
			 {		
					Pump1=0;//关泵
					if(MasterDataBuff[13] == 0)			
					{
							EXValve3=VALVEON;               //排空阀动作
							Delay1000ms();Delay1000ms();Delay1000ms();    //延时1s
							EXValve3=VALVEOFF;            //关阀门
							MasterDataBuff[13] = 0x03;
					} 
			 }
		 
			 
		if((Cup5Status==1) && ((MasterDataBuff[14]==0x01) || (MasterDataBuff[14]==0x02)))//有杯子状态确定是否开启泵 
				Pump5=1;
			 else 
			 {		
					Pump5=0;//关泵
					if(MasterDataBuff[14] == 0)			
					{
							EXValve5=VALVEON;               //排空阀动作
							Delay1000ms();Delay1000ms();Delay1000ms();    //延时1s
							EXValve5=VALVEOFF;            //关阀门
							MasterDataBuff[14] = 0x03;
					} 
			 }
